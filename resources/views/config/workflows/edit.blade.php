...
@section('content')
    <div class="mb-6">
        <a href="{{ route('config.workflows.index') }}" class="text-sm text-primary-700 dark:text-primary-300">&larr; Workflows</a>
        <h1 class="text-2xl font-bold text-[var(--text)]">{{ $workflow ? 'Editar' : 'Novo' }} Workflow</h1>
    </div>

    <form method="POST" action="{{ $workflow ? route('config.workflows.update', $workflow) : route('config.workflows.store') }}"
          x-data="wfForm()" class="bg-[var(--surface)] shadow rounded-lg p-6 space-y-6">
        @csrf
        ...
        <div>
            <label class="block text-sm font-medium text-[var(--text)]">Tipo de entidade</label>
        ...
        <div>
            <label class="block text-sm font-medium text-[var(--text)] mb-2">Status (estados)</label>
            <div class="space-y-2">
                <template x-for="(s, i) in states" :key="i">
                    <div class="flex gap-2 items-center">
                        <input type="text" x-model="s.key" :name="`states[${i}][key]`" placeholder="chave (ex: producao)" class="w-1/3 rounded-md border border-[var(--border)] px-2 py-1 text-sm">
                        <input type="text" x-model="s.label" :name="`states[${i}][label]`" placeholder="Rótulo (ex: Produção)" class="flex-1 rounded-md border border-[var(--border)] px-2 py-1 text-sm">
                        <button type="button" @click="removeState(i)" class="text-red-600 text-sm">Remover</button>
                    </div>
                </template>
            </div>
            <button type="button" @click="addState()" class="mt-2 text-sm text-primary-700 dark:text-primary-300">+ Adicionar status</button>
            <p class="text-xs text-[var(--text-muted)] mt-1">Ao trocar o tipo de entidade, os status padrão são pré-carregados.</p>
        </div>

        <div>
            <label class="block text-sm font-medium text-[var(--text)] mb-2">Transições permitidas</label>
            <div class="space-y-2">
                <template x-for="(t, i) in transitions" :key="i">
                    <div class="flex gap-2 items-center">
                        <select x-model="t.from" :name="`transitions[${i}][from]`" class="w-1/3 rounded-md border border-[var(--border)] px-2 py-1 text-sm">
                        ...
                        <select x-model="t.to" :name="`transitions[${i}][to]`" class="w-1/3 rounded-md border border-[var(--border)] px-2 py-1 text-sm">
                            <template x-for="s in states" :key="s.key">
                                <option :value="s.key" x-text="s.key"></option>
                            </template>
                        </select>
            ...
            <button type="button" @click="addTransition()" class="mt-2 text-sm text-primary-700 dark:text-primary-300">+ Adicionar transição</button>
        </div>
