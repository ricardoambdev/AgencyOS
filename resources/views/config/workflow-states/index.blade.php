...
@section('content')
    <div class="mb-6 flex items-center justify-between">
        <h1 class="text-2xl font-bold text-[var(--text)]">Estados de Workflow</h1>
        <a href="{{ route('config.workflow-states.create', ['entity_type' => $selected]) }}"
           class="rounded-md bg-primary-600 px-3 py-2 text-sm text-white hover:bg-primary-700">Novo estado</a>
    </div>

    <div class="mb-4 flex flex-wrap gap-2">
        @foreach($entityTypes as $type => $label)
            <a href="{{ route('config.workflow-states.index', ['entity_type' => $type]) }}"
               class="rounded-md px-3 py-1 text-sm {{ $type === $selected ? 'bg-primary-600 text-white' : 'bg-[var(--surface)] text-[var(--text)] border border-[var(--border)]' }}">
                {{ $label }}
            </a>
        @endforeach
    </div>
    ...
    <div class="overflow-hidden rounded-lg bg-[var(--surface)] shadow">
        <table class="min-w-full divide-y divide-[var(--border)]">
            <thead class="bg-[var(--surface-2)]">
                <tr class="text-left text-xs font-semibold uppercase text-[var(--text-muted)]">
            ...
            <tbody class="divide-y divide-[var(--border)]">
                @forelse($states as $state)
                    <tr>
                        ...
                        <td class="px-4 py-3 text-right text-sm">
                            <a href="{{ route('config.workflow-states.edit', $state) }}" class="text-primary-700 dark:text-primary-300 hover:underline">Editar</a>
                ...
                @empty
                    <tr><td colspan="7" class="px-4 py-6 text-center text-sm text-[var(--text-muted)]">
