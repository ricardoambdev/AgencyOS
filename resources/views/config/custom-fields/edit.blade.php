...
@section('content')
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-[var(--text)]">Editar Campo Personalizado</h1>
    </div>
    <form method="POST" action="{{ route('config.custom-fields.update', $customField) }}" class="bg-[var(--surface)] shadow rounded-lg p-6">
        @csrf
        @method('PUT')
