        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($items as $item)
                    ...
                    @endphp
                    <tr>
                        <td class="px-4 py-2 text-sm text-gray-500">{{ class_basename($item->entity_type) }}</td>
                        <td class="px-4 py-2 text-sm">
                            @if($route)
                                <a href="{{ $route }}" class="text-indigo-600 hover:underline">
                                    {{ $entity->name ?? ('#' . $item->entity_id) }}
                                </a>
                            @else
                                <span class="text-gray-700">{{ $entity->name ?? ('#' . $item->entity_id) }}</span>
                            @endif
                        </td>
